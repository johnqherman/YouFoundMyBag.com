...
CREATE TABLE public.bags (
  ...
  tag_color_start VARCHAR(7) DEFAULT NULL,
  tag_color_end VARCHAR(7) DEFAULT NULL,
  show_branding BOOLEAN DEFAULT NULL,
  CONSTRAINT bags_tag_color_start_hex CHECK (
    tag_color_start IS NULL
    OR tag_color_start ~ '^#[0-9a-fA-F]{6}$'
  ),
  CONSTRAINT bags_tag_color_end_hex CHECK (
...
CREATE INDEX idx_subscriptions_email_hash ON public.subscriptions (owner_email_hash);
